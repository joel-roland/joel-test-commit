...
# MAGIC %sql
# MAGIC select addr_state, count(loan_status) as count from loan_data_full group by addr_state
...
DELTALAKE_SILVER_PATH = base_table_path + "loan_by_state_delta"

PARQUET_LOCATION = base_table_path + "loan_by_state_parquet"

TABLENAME = username_replaced + ".loan_by_state_delta"

PARQUET_TABLENAME = username_replaced + ".loan_by_state_delta"
...
createDBSQL = f"CREATE DATABASE IF NOT EXISTS {username_replaced}" 
spark.sql(createDBSQL)

# COMMAND ----------

cleanupSQL = f"DROP TABLE IF EXISTS {TABLENAME}" 
spark.sql(cleanupSQL)

# COMMAND ----------

sqlCreate = f"create table {TABLENAME} using DELTA location '{DELTALAKE_SILVER_PATH}' AS SELECT * FROM loan_by_state" 
spark.sql(sqlCreate)

# COMMAND ----------

df = spark.sql(f"SELECT * FROM {TABLENAME}")
df.createOrReplaceTempView("loan_by_state_delta_t")

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from loan_by_state_delta_t

# COMMAND ----------

display(spark.sql(f"DESCRIBE DETAIL delta.`{DELTALAKE_SILVER_PATH}`"))
...
while i <= 6:
  ...
  insert_sql = "INSERT INTO " + TABLENAME + " VALUES ('IA', 4500)"
...
# MAGIC select addr_state, sum(`count`) as loans from loan_by_state_delta_t group by addr_state
...
lbs_df = sql("select * from loan_by_state_delta_t")

# Save DataFrame to Parquet
lbs_df.write.mode("overwrite").parquet(PARQUET_LOCATION) 

# Reload Parquet Data
lbs_pq = spark.read.parquet(PARQUET_LOCATION)

# Create new view on this parquet data
lbs_pq.createOrReplaceTempView("loan_by_state_pq")

sqlCreate = f"DROP TABLE IF EXISTS '{PARQUET_TABLENAME}'"
spark.sql(sqlCreate)

sqlCreate = f"create table '{PARQUET_TABLENAME}' using PARQUET LOCATION '{PARQUET_LOCATION}' AS SELECT * FROM loan_by_state_pq" 
spark.sql(sqlCreate)

# Review data
display(sql("select * from '{PARQUET_TABLENAME}'"))
...
spark.sql(f"DELETE FROM {TABLENAME} WHERE addr_state = 'IA'")
...
# MAGIC The data was originally supposed to be assigned to `WA` state, so let's `UPDATE` those values to Washington
...
spark.sql(f"UPDATE {TABLENAME} SET `count` = 27000 WHERE addr_state = 'WA'")
...
mergeSQL =   f"MERGE INTO {TABLENAME} as d" + \
...
loans = sql("select addr_state, cast(rand(10)*count as bigint) as count, cast(rand(10) * 10000 * count as double) as amount from loan_by_state_delta_t")
display(loans)
...
display(spark.sql(f"select addr_state, sum(`amount`) as amount from {TABLENAME} group by addr_state order by sum(`amount`) desc limit 10"))
...
display(spark.sql(f"SELECT * FROM {TABLENAME} VERSION AS OF 0"))

# COMMAND ----------

display(spark.sql(f"SELECT * FROM {TABLENAME} VERSION AS OF 9"))
